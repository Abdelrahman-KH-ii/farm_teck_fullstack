                ...
                <Card className="mb-6 bg-gradient-to-r from-primary/5 to-accent/5 border-primary/20">
                  <CardContent className="pt-6">
                    <div className="flex flex-col md:flex-row items-center gap-6">
                      <div className="text-center">
                        <p className="text-sm font-semibold text-muted-foreground mb-1">Overall Soil Health Score</p>
                        <p className={`text-6xl font-black ${statusColor}`}>{soilData.health_score}</p>
                        <p className={`text-lg font-bold mt-1 ${statusColor}`}>{soilData.status_label}</p>
                      </div>
                      <div className="flex-1 grid grid-cols-2 gap-3">
                        {soilData.nearest_field && (
                          <div className="col-span-2 p-3 bg-background/60 rounded-lg border border-border text-sm text-muted-foreground">
                            📍 Based on nearest field: <strong className="text-foreground capitalize">{soilData.nearest_field.crop}</strong> crop 
                            ({soilData.nearest_field.year}) at {soilData.nearest_field.lat.toFixed(3)}, {soilData.nearest_field.lon.toFixed(3)}
                          </div>
                        )}
                        {soilData.fertility_index !== null && (
                          <div className="p-3 bg-background/60 rounded-lg border border-border">
                            <p className="text-xs text-muted-foreground">Fertility Index</p>
                            <p className="font-bold text-foreground">{soilData.fertility_index?.toFixed(3)}</p>
                          </div>
                        )}
                        {soilData.aridity_index !== null && (
                          <div className="p-3 bg-background/60 rounded-lg border border-border">
                            <p className="text-xs text-muted-foreground">Aridity Index</p>
